...
class Environment:
    def __init__ (self, args, draw_it = False, n_UAV = 5, time_step = 0.1, TTI = 0.01):
        ...
        self.veh_cpu_max = 2 # GHz
        self.veh_cpu_min = 1 # GHz
        self.uav_cpu = 10 
        ...
        self.task_lambda_poss = [0.6, 0.3, 0.1] # 每种类型的出现概率
    ...
    def initialize(self, bound):
        ...
        # 整个区域是[500, 2000; 250, 1250]=1500 * 1000, 500一个格子，分别是6个
        # 1: [500, 1000; 250, 750] # BS 1 在中心点 750, 500
        # 2: [1000, 1500; 250, 750] # UAV 1 中心点 1250, 500
        # 3: [500, 1000; 750, 1250] # UAV 2 中心点 750, 1000
        # 4: [1000, 1500; 750, 1250] # UAV 3 中心点 1250, 1000
        # 5: [1500, 2000; 250, 750] # UAV 4 在中心点 1750, 500
        # 6: [1500, 2000; 750, 1250] # BS 2 在中心点 1750, 1000 
        # 中间的2~5是UAV,初始化在四个区域
        self.x_range = [[1000, 1500], [500, 1000], [1000, 1500], [1500, 2000]]
        self.y_range = [[250, 750], [750, 1250], [750, 1250], [250, 750]]
        self.max_range_x = 2000
        self.min_range_x = 500
        self.max_range_y = 1250
        self.min_range_y = 250
        grid_width = self.args.grid_width
        x_grid_num = int((self.max_range_x - self.min_range_x) / grid_width)
        y_grid_num = int((self.max_range_y - self.min_range_y) / grid_width)
        # 生成一个map，按照上述网格，按照高斯分布获取概率
        if self.max_serving_vehicles == 60:
            # 均匀分布, 0.37
            self.hot_spot_pdf = np.ones((x_grid_num, y_grid_num)) * 0.37
        elif self.max_serving_vehicles > 60:
            # 在min_x和max_x之间随机生成一个数, min_y和max_y之间随机生成一个数, 作为hot spot的位置
            # hot_spot_position1 = [np.random.rand() * 1500 + 500, np.random.rand() * 1000 + 250]
            # hot_spot_position2 = [np.random.rand() * 1500 + 500, np.random.rand() * 1000 + 250]
            hot_spot_position1 = [1500, 750]
            hot_spot_position2 = [750, 500]
            # 以两个hot spot为中心，std = 1, x_grid_num * y_grid_num的网格中，生成一个高斯分布的pdf
            x = np.linspace(self.min_range_x, self.max_range_x, x_grid_num)
            y = np.linspace(self.min_range_y, self.max_range_y, y_grid_num)
            X, Y = np.meshgrid(x, y)
            Z1 = np.exp(-(((X - hot_spot_position1[0])/grid_width)**2 + ((Y - hot_spot_position1[1])/grid_width)**2) / (2 * 3**2))
            Z2 = np.exp(-(((X - hot_spot_position2[0])/grid_width)**2 + ((Y - hot_spot_position2[1])/grid_width)**2) / (2 * 3**2))
            Z = Z1 + Z2
            # Z = Z / np.sum(Z)
            self.hot_spot_pdf = Z
        else:
            # hot_spot_position = [np.random.rand() * 1500 + 500, np.random.rand() * 1000 + 250]
            hot_spot_position = [750, 500]
            x = np.linspace(self.min_range_x, self.max_range_x, x_grid_num)
            y = np.linspace(self.min_range_y, self.max_range_y, y_grid_num)
            X, Y = np.meshgrid(x, y)
            Z = np.exp(-(((X - hot_spot_position[0])/grid_width)**2 + ((Y - hot_spot_position[1])/grid_width)**2) / (2 * 3**2))
            # Z = Z / np.sum(Z)
            self.hot_spot_pdf = Z
        self.initialize_BSs()
    ...
    def initialize_BSs(self):
        self.BS_positions = []
        self.BSs = []
        pos_bss = [ [750, 500], [1750,1000]]
        for i in range(self.n_BS):
            # 在min_x和max_x之间随机生成一个数, min_y和max_y之间随机生成一个数, 作为BS的位置
            pos_bs = pos_bss[i]
            self.BS_positions.append(pos_bs)
    ...
    def initialize_UAVs(self, vehicle_positions = None):
        ...
        if vehicle_positions is not None:
            # kmeans = KMeans(n_clusters=self.n_UAV).fit(vehicle_positions)
            ...
            # 先UAV，后BS的中心点，用于控制weighted voronoi图
            self.area_centers = [[1250, 500], [750, 1000], [1250, 1000], [1750, 500], [750, 500], [1750, 1000]]
            assert len(self.x_range) == self.n_UAV
            for uid in range(self.n_UAV):
                pos_uav = [self.between(self.x_range[uid][0], self.x_range[uid][1]), self.between(self.y_range[uid][0], self.y_range[uid][1])]
                # 保证所在的位置%50==0，即在网格中心
                pos_uav[0] = int(pos_uav[0] / 50) * 50
                pos_uav[1] = int(pos_uav[1] / 50) * 50
                uav_i = UAV(uid, self.hei_UAV, pos_uav, 0, 0, self.uav_cpu)
                self.UAVs.append(uav_i)
            # ----------------------------------------------------------------------------------
            # self.area_centers = np.zeros((self.n_UAV + self.n_BS, 2))
            # for uid in range(self.n_UAV):
            #     self.area_centers[uid] = self.UAVs[uid].position
            # for bsid in range(self.n_BS):
            #     self.area_centers[self.n_UAV + bsid] = self.BSs[bsid].position
    def generate_vehicle(self, row_data):
        ...
        if len(self.serving_vehicles) < self.max_serving_vehicles: # 服务车辆
            ...
            veh = Vehicle(row_data['id'], [row_data['x'], row_data['y']], row_data['angle'], row_data['speed'], 0, serving = False, task_lambda = 0)
            vehicle = veh
            x_grid_max = self.hot_spot_pdf.shape[0]
            y_grid_max = self.hot_spot_pdf.shape[1]
            veh_grid_x = max(0, min(int((vehicle.position[0]-self.min_range_x) / self.args.grid_width), x_grid_max-1))
            veh_grid_y = max(0, min(int((vehicle.position[1]-self.min_range_y) / self.args.grid_width), y_grid_max-1))
            vehicle.task_lambda = np.clip(self.hot_spot_pdf[veh_grid_x, veh_grid_y] * (10), 1, 10)
    ...
    def renew_uav_positions(self, directions, speeds):
        '''输入每个无人机的移动方向和速度'''
        # 假设directions和speeds是list
        for uid in range(len(self.UAVs)):
            self.UAVs[uid].update_direction(directions[uid])
            self.UAVs[uid].update_velocity(speeds[uid])
            self.UAVs[uid].update_position(self.time_step)
        flags = self.check_bound()
        self.update_distances()
        # self.area_centers 更新为无人机的位置
        # for uid in range(self.n_UAV):
        #     self.area_centers[uid] = self.UAVs[uid].position
        return flags
        ...
        for v_id in tmp_keys:
            ...
            if row_data is not None:
                ...
                if not vehicle.serving:
                    ...
                    veh_grid_x = max(0, min(int((vehicle.position[0]-self.min_range_x) / self.args.grid_width), x_grid_max-1))
                    veh_grid_y = max(0, min(int((vehicle.position[1]-self.min_range_y) / self.args.grid_width), y_grid_max-1))
                    vehicle.task_lambda = self.hot_spot_pdf[veh_grid_x, veh_grid_y] * (10)
                    # vehicle.task_lambda = int(vehicle.velocity) / 2
                    # if not vehicle.served_last_period:
                    #     vehicle.task_lambda = vehicle.task_lambda + 1
                    # else:
                    #     vehicle.task_lambda = vehicle.task_lambda - 1
                    vehicle.served_last_period = False
                    vehicle.task_lambda = max(1, vehicle.task_lambda)
                    vehicle.task_lambda = min(10, vehicle.task_lambda)
                # 删除已经更新的车辆
                del rows[v_id]
            else:
    ...
    def generate_tasks(self):
        ...
        for idx, vi in enumerate(v_by_idx):
            ...
            for tti_slot in range(len(time_slot_task)):
                ...
                tmp_task = Task(self.task_id_cnt, vi, tti_slot*self.TTI+self.cur_time, between(self.task_cpu_min, self.task_cpu_max), between(self.task_data_min, self.task_data_max), between(self.task_ddl_min, self.task_ddl_max))
                self.task_id_cnt += 1
    ...
    def Check_To_X_Tasks(self):
        ...
        for task in self.to_offload_tasks.copy():
            # if task.start_time + task.ddl <= self.cur_time:
            self.to_offload_tasks.remove(task)
            self.failed_tasks.append({
                'task':task,
        ...
        # 然后检查to_verify_tasks
        for task in self.to_verify_tasks.copy():
            if task.start_time + task.ddl <= self.cur_time:
                self.to_verify_tasks.remove(task)
                self.failed_tasks.append({
                    'task':task,
                    'reason':Task.OUT_OF_DDL
                })
        # 然后检查to_pay_tasks，如果超过了ddl，那么就直接失败，但是不需要加到failed_tasks
        # 这个是dict
        # for task_id in list(self.to_pay_tasks.keys()).copy():
        #     tasks = self.to_pay_tasks[task_id]
        ...
        #     else:
        #         self.to_pay_tasks[task_id] = tasks
    ...
    def get_rewards(self, v2u_assigned, v2i_assigned):
        ...
        return 100 * ((balance_omega * tot_qos + (1 - balance_omega) * (np.min(tot_qos)) * np.ones_like(tot_qos)) - flying_pen)

    def get_discrete_rewards(self, states, UAV_positions=None):
        # 40 * 40的map，分别表示了包含的cpu资源，请求任务，需要传输的数据量，以及分配给哪个设备
        # 对于分配给一个设备的所有区域，假设cpu资源全部用来完成请求任务，以此计算“计算时延”
        # 对于分配给一个设备的所有区域，假设静态节点，只计算path loss，并且相邻的格点视作50m的距离。分别用0-5为v2u信道，6-9为v2i信道来计算rate
        # 带宽分配为每一个设备的区域内全频段频分，全部有self.bandwidth的带宽，区域内有多少个任务数量
        cpu_map = states[0, :, :]
        req_map = states[1, :, :]
        trans_map = states[2, :, :]
        ass_map = states[3, :, :]
        task_cnt_map = copy.deepcopy(req_map)
        req_map = req_map * (self.task_cpu_max+self.task_cpu_min) / 2
        region_comp = np.zeros(self.n_UAV+self.n_BS)
        region_trans = np.zeros(self.n_UAV+self.n_BS)
        region_task_cnt = np.zeros(self.n_UAV+self.n_BS)
        vehicles = np.array(list(self.vehicles.values()))
        positions = np.array([v.position for v in vehicles])
        istask_v = np.array([1 if v.task_lambda > 0 else 0 for v in vehicles])
        task_v_map = np.zeros((40, 40))
        dx_dy = (positions - np.array([self.min_x, self.min_y])) / np.array([self.max_x - self.min_x, self.max_y - self.min_y]) - 1e-3
        dx_dy = np.floor(dx_dy * np.array([40, 40])).astype(int)
        np.add.at(task_v_map, tuple(dx_dy.T), istask_v)
        
        # UAV_positions = np.array([uav.position for uav in self.UAVs])
        # UAV_positions = np.clip(UAV_positions, np.array([self.min_x, self.min_y]), np.array([self.max_x, self.max_y]))        
        # d_UAV_positions = np.floor(((UAV_positions - np.array([self.min_x, self.min_y])) / np.array([self.max_x - self.min_x, self.max_y - self.min_y])) * np.array([40, 40]))
        
        # y, x = np.meshgrid(np.arange(40), np.arange(40))
        # dx = x - d_UAV_positions[:, None, None, 0]
        # dy = y - d_UAV_positions[:, None, None, 1]
        # distance_UAV = np.sqrt(dx**2 + dy**2) * 50 + 50
        region_task_v = np.zeros_like(region_trans)
        avg_cpu = (self.task_cpu_max + self.task_cpu_min) / 2
        for uav_id in range(self.n_UAV):
            ass_idx = ass_map == uav_id
            task_cnt = np.sum(task_cnt_map[ass_idx])
            cpu_ass = np.sum(cpu_map[ass_idx])
            req_ass = np.sum(req_map[ass_idx])
            region_task_cnt[uav_id] = task_cnt
            if task_cnt > 0 and cpu_ass > 0:
                task_v_cnt = np.sum(task_v_map[ass_idx]) + 0.1
                region_task_v[uav_id] = task_v_cnt
                region_trans[uav_id] = sum(trans_map[ass_idx]) / task_v_cnt # 所有区域内部平均每个车辆的传输时延
                region_comp[uav_id] = req_ass / cpu_ass * avg_cpu * task_cnt / task_v_cnt 
        for bs_id in range(self.n_BS):
            ass_idx = ass_map == self.n_UAV + bs_id
            task_cnt = np.sum(task_cnt_map[ass_idx])
            cpu_ass = np.sum(cpu_map[ass_idx])
            req_ass = np.sum(req_map[ass_idx])
            region_task_cnt[self.n_UAV + bs_id] = task_cnt
            if task_cnt > 0 and cpu_ass > 0:
                task_v_cnt = np.sum(task_v_map[ass_idx]) + 0.1
                region_task_v[self.n_UAV + bs_id] = task_v_cnt
                region_trans[self.n_UAV + bs_id] = sum(trans_map[ass_idx]) / task_v_cnt 
                region_comp[self.n_UAV + bs_id] = req_ass / cpu_ass * avg_cpu * task_cnt / task_v_cnt

        # region_comp = np.clip(region_comp, 0, 5)
        # 对于覆盖区域内部，每一个网格，距离的倒数*需要的时延，作为reward
        rewards = np.zeros_like(region_comp)
        for i in range(self.n_UAV):
            ass_idx = (ass_map == i)  * (trans_map!=0)
            ass_region_time = region_comp[i] + region_trans[i] 
            # + 1 / ((distance_UAV[i] / 50)[20,20])
            rewards[i] = ass_region_time - 0.05 * self.UAVs[i].velocity / self.uav_max_speed
        
        self.avg_qos = (np.sum((region_comp+region_trans)*region_task_v)) / (np.sum(region_task_v)+0.1)
        # self.avg_qos = max(region_comp + region_trans)
        rewards[rewards>1] = 1
        rewards += 1 / (self.avg_qos+0.1)
        return rewards * 100


    def get_states(self, uav_weight, bs_weight, map_width, map_height, UAV_positions = None):
        # 循环车辆，填充计算资源分布。无人机控制上下左右，直接移动网格。再返回一张map表示各个网格区域被占据的情况
        grid_length = 2000 // map_width
        cpu_maps = np.zeros((map_width, map_height))
        req_maps = np.zeros((map_width, map_height))
        vehicles = np.array(list(self.vehicles.values()))
        positions = np.array([v.position for v in vehicles])
        dx_dy = (positions - np.array([self.min_x, self.min_y])) / np.array([self.max_x - self.min_x, self.max_y - self.min_y]) - 1e-3
        dx_dy = np.floor(dx_dy * np.array([map_width, map_height])).astype(int)
        vcpu = np.array([v.CPU_frequency for v in vehicles])
        vreq = np.array([v.task_lambda for v in vehicles])
        istask_v = np.array([1 if v.task_lambda > 0 else 0 for v in vehicles])
        task_v_maps = np.zeros((map_width, map_height))
        np.add.at(cpu_maps, tuple(dx_dy.T), vcpu)
        np.add.at(req_maps, tuple(dx_dy.T), vreq)
        np.add.at(task_v_maps, tuple(dx_dy.T), istask_v)

        assigned_map = -np.ones((map_width, map_height))
        if UAV_positions is None:
            # UAV_positions = np.array([self.UAVs[i].position for i in range(self.n_UAV)])
            UAV_positions = np.array([self.area_centers[i] for i in range(self.n_UAV)])
        UAV_positions = np.clip(UAV_positions, np.array([self.min_x, self.min_y]), np.array([self.max_x, self.max_y]))        
        d_UAV_positions = np.floor(((UAV_positions - np.array([self.min_x, self.min_y])) / np.array([self.max_x - self.min_x, self.max_y - self.min_y])) * np.array([map_width, map_height]))
        # BS_positions = np.array([self.BSs[i].position for i in range(self.n_BS)])
        BS_positions = np.array([self.area_centers[i+self.n_UAV] for i in range(self.n_BS)])
        d_BS_positions = np.floor(((BS_positions - np.array([self.min_x, self.min_y])) / np.array([self.max_x - self.min_x, self.max_y - self.min_y])) * np.array([map_width, map_height]))
        d_UAV_positions = np.clip(d_UAV_positions, 0, map_width-1)
        d_BS_positions = np.clip(d_BS_positions, 0, map_width - 1)
        for pos in d_UAV_positions:
            pos = np.array(pos, dtype = np.int16)
            cpu_maps[pos[0], pos[1]] += self.uav_cpu
            
        for pos in d_BS_positions:
            pos = np.array(pos, dtype = np.int16)
            cpu_maps[pos[0], pos[1]] += self.BS_cpu
        d_dist_pos = np.concatenate([d_UAV_positions, d_BS_positions], axis=0) # [env.n_BS+env.n_UAV, 2]
        v_weight = np.concatenate([uav_weight, bs_weight], axis=0)
        dist_thr = np.concatenate([200//50*np.ones(self.n_UAV), 200//50*np.ones(self.n_BS)])
        y, x = np.meshgrid(np.arange(map_width), np.arange(map_height))
        ...
        # 计算所有格点到各设备的距离
        distances = np.sqrt((d_dist_pos[:, 0] - x)**2 + (d_dist_pos[:, 1] - y)**2)

        # 计算各格点的权重
        distances_weighted  = distances - v_weight[np.newaxis, np.newaxis, :] 

        # 寻找距离最小（权重最小）的设备，生成assigned_map
        assigned_map = np.argmin(distances_weighted, axis=-1)

        # 填入距离超过阈值的格点
        # assigned_map_distances = distances[np.arange(map_width)[:, None, None], np.arange(map_height)[:, None], assigned_map[:,:,None]].squeeze(-1)  # shape: (map_width, map_height)
        # assigned_map_dist_thr = dist_thr[assigned_map]  # shape: (map_width, map_height)
        # assigned_map[assigned_map_distances > assigned_map_dist_thr] = -1
        # assigned_map = np.where(assigned_map==-1, self.n_UAV+np.argmin(distances_weighted[:, :, self.n_UAV:], axis=-1), assigned_map) # 失败的分配给基站
        # 把vehicle_by_index里面的车辆，根据位置对应到assigned_map，然后vehicle.assigned_to = assigned_map
        for vidx in range(len(vehicles)):
            grid_vx = dx_dy[vidx][0]
            grid_vy = dx_dy[vidx][1]
            vehicles[vidx].assigned_to = assigned_map[grid_vx, grid_vy]

        PL_map = 300 * np.ones((map_width, map_height)) # 默认pl-300
        # 获取距离矩阵
        y, x = np.meshgrid(np.arange(map_width), np.arange(map_height))
        dx = x - d_UAV_positions[:, None, None, 0]
        dy = y - d_UAV_positions[:, None, None, 1]
        distance_UAV = np.sqrt(dx**2 + dy**2) * grid_length + grid_length

        dx = x - d_BS_positions[:, None, None, 0]
        dy = y - d_BS_positions[:, None, None, 1]
        distance_BS = np.sqrt(dx**2 + dy**2) * grid_length + grid_length

        # 计算路径损耗
        PL_UAV = V2UChannel.calculate_path_loss(distance_UAV)
        PL_BS = V2IChannel.calculate_path_loss(distance_BS)
        signal_map = np.zeros_like(PL_map)
        # 根据ass_map的值选择使用UAV或者BS的路径损耗
        band_map = self.bandwidth * np.ones_like(PL_map) # 对于同一个区域的设备，计算所有的任务数量，然后用band/任务数量作为分配的带宽
        avg_data = (self.task_data_max + self.task_data_min) / 2
        for uav_id in range(self.n_UAV):
            ass_idx = assigned_map == uav_id
            PL_map[ass_idx] = PL_UAV[uav_id, ass_idx]
            signal_map[ass_idx] = self.V2U_power_dB
            task_cnt = np.sum(req_maps[ass_idx]) # 区域内的数量
            if task_cnt > 0:
                atest = sum(task_v_maps[ass_idx])+0.1
                band_map[ass_idx] = band_map[ass_idx] / atest * task_v_maps[ass_idx]   # 区域内部有多少个车辆,直接带宽相加

        b2c_time = np.zeros(self.n_BS)
        for bs_id in range(self.n_BS):
            ass_idx = assigned_map == self.n_UAV + bs_id
            PL_map[ass_idx] = PL_BS[bs_id, ass_idx]
            signal_map[ass_idx] = self.V2I_power_dB
            task_cnt = np.sum(req_maps[ass_idx]) # 区域内的数量
            if task_cnt > 0:
                atest = sum(task_v_maps[ass_idx])+0.1
                band_map[ass_idx] = band_map[ass_idx] / atest * task_v_maps[ass_idx] 
                b2c_time[bs_id] = np.mean((avg_data * req_maps[ass_idx] / (task_v_maps[ass_idx]+0.1)) / (self.BS2C_rate / atest)) # 区域内平均每一个任务车辆的任务数据大小 / 平均每一个车辆的传输速率
        data_maps_t = np.zeros_like(req_maps)
        data_maps_t[req_maps!=0] = avg_data * req_maps[req_maps!=0] # 每个网格内部的总的数据数量
        SNR_map = np.divide(10 ** ((signal_map - PL_map) / 10), self.sig2)
        rate_map = np.clip(band_map * np.log2(1 + SNR_map), 1e-5, 300) # 每个区域内平均每一个任务车辆的传输速率
        trans_map = np.divide(data_maps_t, rate_map)  # 每个区域内部的总体传输时延(求和)
        for bs_id in range(self.n_BS):
            ass_idx = assigned_map == self.n_UAV + bs_id
            trans_map[ass_idx] += b2c_time[bs_id]

        assigned_map = assigned_map[np.newaxis, :, :] / 5
        cpu_maps = cpu_maps[np.newaxis, :, :] / 2.5
        req_maps = req_maps[np.newaxis, :, :] / 10
        states = np.concatenate([cpu_maps, req_maps, assigned_map], axis = 0)
        states = np.repeat(states[np.newaxis, :, :, :], self.n_UAV + self.n_BS, axis = 0)
        pos_states = np.concatenate([d_UAV_positions, d_BS_positions], axis = 0)
        return states, pos_states


    def get_states_as_vector(self, v_num = 50, bs_num = 4, uav_num = 6, vehicle_ids = None, vehicle_positions = None):
        # if vehicle_ids is None:
        #     vehicle_ids = list(self.vehicles.keys())
        # vehicle_ids = vehicle_ids[:v_num]
        sorted_uavs = self.UAVs[:uav_num]
        sorted_bs = self.BSs[:bs_num]
        v2ui_assigned = np.zeros((v_num))
        v2ui_assigned = np.repeat(v2ui_assigned[np.newaxis, :], self.n_UAV + self.n_BS, axis = 0)
        uav_idm = np.zeros((self.n_UAV+self.n_BS, 1))
        vdirection2uav = -np.ones((self.n_UAV + self.n_BS, v_num))
        vdistance2uav = -np.ones((self.n_UAV + self.n_BS, v_num))
        udirection2uav = -np.ones((self.n_UAV + self.n_BS, uav_num))
        udistance2uav = -np.ones((self.n_UAV + self.n_BS, uav_num))
        bsdirection2uav = -np.ones((self.n_UAV + self.n_BS, bs_num))
        bsdistance2uav = -np.ones((self.n_UAV + self.n_BS, bs_num))
        vcomp = np.zeros((self.n_UAV + self.n_BS, v_num))
        ucomp = np.zeros((self.n_UAV + self.n_BS, uav_num))
        bscomp = np.zeros((self.n_UAV + self.n_BS, bs_num))

        avg_cpu = (self.task_cpu_max + self.task_cpu_min) / 2
        avg_ddl = (self.task_ddl_max + self.task_ddl_min) / 2
        veh_comp = np.zeros(v_num)
        sorted_vehicle = list(self.vehicles.values()) # 选取距离最近的50个车辆作为观测，部分可观测
        for i, uav in enumerate(self.UAVs):
            uav_idm[i] = i
            # UAVs
            # x1, y1 = uav.position
            tmp_sort = sorted(sorted_vehicle, key=lambda x:self.distance(x.position, uav.position))
            tmp_sort = tmp_sort[:v_num]
            
            for tidx, veh in enumerate(tmp_sort):
                veh_comp[tidx] = veh.CPU_frequency - avg_cpu * veh.task_lambda / avg_ddl
                if veh.assigned_to != -1:
                    v2ui_assigned[:, tidx] = veh.assigned_to
            x1, y1 = 0, 0
            # Vehicles
            positions = np.array([veh.position for veh in tmp_sort])
            if len(positions.shape) > 1:
                x2, y2 = positions[:,0], positions[:,1]
                x2, y2 = x2-x1, y2-y1
                vdistance2uav[i, :len(x2)] = (x2 - self.min_x) 
                vdirection2uav[i, :len(y2)] = (y2 - self.min_y) 
                vcomp[i, :] = veh_comp
            # udistance2uav[i, 0] = (x1 - self.min_x) / (self.max_x - self.min_x)
            # udirection2uav[i, 0] = (y1 - self.min_y) / (self.max_y - self.min_y) 
            # ucomp[i, 0] = uav.CPU_frequency
            cnt = 0
            for j, other_uav in enumerate(sorted_uavs):
                x2, y2 = other_uav.position
                x2, y2 = x2-x1, y2-y1
                udistance2uav[i, cnt] = (x2 - self.min_x) 
                udirection2uav[i, cnt] = (y2 - self.min_y) 
                ucomp[i, cnt] = other_uav.CPU_frequency
                cnt += 1
                
            # Base stations
            for j, bs in enumerate(sorted_bs):
                x2, y2 = bs.position
                x2, y2 = x2-x1, y2-y1
                bsdistance2uav[i, j] = (x2 - self.min_x) 
                bsdirection2uav[i, j] = (y2 - self.min_y) 
                bscomp[i, j] = bs.CPU_frequency
        for ij, bs in enumerate(self.BSs):
            i = ij +self.n_UAV
            uav_idm[i] = i
            # UAVs
            # x1, y1 = bs.position
            tmp_sort = sorted(sorted_vehicle, key=lambda x:self.distance(x.position, bs.position))
            tmp_sort = tmp_sort[:v_num]
            ...
            x1, y1 = 0, 0
            # Vehicles
            if vehicle_positions != None:
                positions = np.array(vehicle_positions)[:v_num]
            else:                
                positions = np.array([veh.position for veh in tmp_sort])
            # 获取vehicle的计算资源和任务需求，表现为正负数
            if len(positions.shape) > 1:
                x2, y2 = positions[:,0], positions[:,1]
                x2, y2 = x2-x1, y2-y1
                vdistance2uav[i, :len(x2)] = (x2 - self.min_x)
                vdirection2uav[i, :len(y2)] = (y2 - self.min_y) 
                vcomp[i, :v_num] = veh_comp[:v_num]
            cnt = 0
            for j, other_uav in enumerate(sorted_uavs):
                x2, y2 = other_uav.position
                x2, y2 = x2-x1, y2-y1
            ...
            for j, bs in enumerate(sorted_bs):
                ...
                bsdirection2uav[i, j] = (y2 - self.min_y) 
                bscomp[i, j] = bs.CPU_frequency
        return np.concatenate([vdirection2uav, vdistance2uav, vcomp, udirection2uav, udistance2uav, ucomp, bsdirection2uav, bsdistance2uav, bscomp, v2ui_assigned, uav_idm], axis=1)
